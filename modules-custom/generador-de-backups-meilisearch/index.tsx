...
export default function BackupDropboxMeilisearchModule() {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      if (configForm.meilisearch_url.trim()) payload.meilisearch_url = configForm.meilisearch_url.trim();
      const res = await fetch(`${BASE}/config`, {
        method: 'PUT',
  ...
  const cleanupHistory = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/cleanup`, { method: 'POST' });
      const json = await res.json();
      if (!json.ok) {
      ...
      <div className="space-y-4">
        ...
        {activeTab === 'config' && (
          <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Meilisearch URL</label>
              <input
                type="text"
                placeholder="http://localhost:7700"
                className="w-full max-w-md px-3 py-2 border border-gray-300 rounded-lg text-sm"
                value={configForm.meilisearch_url || config.meilisearch_url || ''}
                onChange={(e) => setConfigForm((f) => ({ ...f, meilisearch_url: e.target.value }))}
              />
            </div>
            <div>
        ...
        {activeTab === 'docs' && (
          <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3 text-sm text-gray-700">
            <p>Este modulo crea un dump de Meilisearch via SSH y lo sube a Dropbox.</p>
            <div>
...
0 0 * * * curl -s -X POST "https://workers.zeroazul.com/api/custom-module9/backup-dropbox-meilisearch/run"
              </pre>
