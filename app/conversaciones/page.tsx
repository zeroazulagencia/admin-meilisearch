...
export default function Conversaciones() {
  ...
  // Mensajes pendientes/enviados localmente (para mostrar inmediatamente en el chat)
  ...
  const handleSendMessage = async () => {
    ...
    // Agregar mensaje inmediatamente al chat con estado "enviando"
    ...
    try {
      const res = await fetch('/api/whatsapp/send-message', {
        ...
        body: JSON.stringify({
          ...
          message_type: 'text',
          message: messageText,
          user_id: selectedConversation.user_id, // Pasar user_id para guardar en Meilisearch
          phone_number_id: selectedConversation.phone_number_id || selectedConversation.user_id // Pasar phone_number_id para guardar en Meilisearch
        })
      });
      ...
      if (data.ok && data.data) {
        // Actualizar el mensaje a "enviado" con el message_id de WhatsApp
        ...
        console.error('[CONVERSACIONES] Error al enviar mensaje:', data.error || 'Error desconocido');
      }
    } catch (e: any) {
      console.error('[CONVERSACIONES] Error enviando mensaje:', e);
      // Actualizar el mensaje a "error"
      setPendingMessages(prev => prev.map(msg => 
        msg.id === messageId 
          ? { ...msg, status: 'error' as const }
  ...
  useEffect(() => {
    if (selectedConversation && currentAgentDetails) {
      ...
      setHumanModeStatus(null);
      setMessageInput('');
      setPendingMessages([]); // Limpiar mensajes pendientes al cambiar de conversación
    }
              ...
              <div className="flex-1 flex flex-col bg-gray-100">
                {selectedConversation ? (
                  <>
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => {
                              loadConversations();
                              setPendingMessages([]); // Limpiar mensajes pendientes al recargar
                            }}
                    ...
                    >
                      {/* Mostrar mensajes pendientes/enviados primero */}
