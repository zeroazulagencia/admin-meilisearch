    ...
    try {
      ...
      if (data.ok) {
        ...
        setPendingMessages(prev => prev.map(msg => 
          msg.id === messageId 
            ? { ...msg, status: 'sent' as const, messageId: data.message_id }
            : msg
        ));
        
        // Recargar conversaciones después de un breve delay para que aparezca en la BD
        setTimeout(() => {
          loadConversations();
          // Limpiar mensajes pendientes después de recargar (ya estarán en la conversación)
          setTimeout(() => {
            setPendingMessages([]);
          }, 2000);
        }, 1500);
                              ...
                              <div className="flex justify-end mb-2">
                                <div className="max-w-[70%] bg-green-500 text-white rounded-2xl px-3 py-2 shadow-sm" style={{ 
                                  ...
                                  <div className="flex items-center justify-end gap-1 mt-1">
                                    ...
                                    {/* Mostrar doble check para mensajes enviados */}
