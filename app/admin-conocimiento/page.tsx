...
export default function AdminConocimiento() {
  ...
  const [isFromWeb, setIsFromWeb] = useState(false);
  ...
  const [activeTab, setActiveTab] = useState<'agentes'>('agentes');
            ...
            if (isEmpty) {
              ...
              if (fieldInfo.type === 'array') {
                ...
                if (!extractedValue) {
                  ...
                  // Intentar inferir datetime (fecha actual si no hay información)
                  if (fieldNameLower.includes('datetime') || fieldNameLower.includes('fecha') || fieldNameLower.includes('date')) {
                    extractedValue = new Date().toISOString();
                  }
                }
        ...
        return (
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900">
              {isClient ? 'Conocimiento' : 'Admin Conocimiento'}
            </h1>
            {!isClient && (
              <button
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
              <p className="text-gray-500">
                {(() => {
                  ...
                  const isClient = permissions?.type !== 'admin';
                  return isClient 
                    ? `${selectedAgent?.name} no tiene conocimientos asociados.`
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex-1 overflow-auto">
              {/* Opción para cargar PDF - Solo mostrar si no viene de web */}
              {!isFromWeb && (
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
