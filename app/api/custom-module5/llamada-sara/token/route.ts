...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!ROOM_REGEX.test(room)) {
      return NextResponse.json({ ok: false, error: 'Sala inválida' }, { status: 400 });
    }

    const [rows] = await query<any>('SELECT id FROM modulos_sara_11_llamadas WHERE room_id = ? AND estado = ?', [room, 'activa']);
    if (!rows.length) {
    ...
    const config: Record<string, string> = Object.fromEntries(cfg.map((r: any) => [r.config_key, r.config_value ?? '']));
    ...
    const token = new AccessToken(
      ...
      { identity: 'participante', ttl: 3600 }
    );
