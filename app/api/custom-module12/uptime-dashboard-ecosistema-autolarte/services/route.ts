...
import { getCredentials, getModuleConfig } from '@/modules-custom/uptime-dashboard-ecosistema-autolarte/utils/config';
...
async function checkHttpService(url: string, timeout: number = 5, options: {
  ...
  try {
    ...
    const fetchOptions: RequestInit = {
      ...
      headers: {
        'User-Agent': 'Autolarte-StatusPanel/1.0',
        ...headers
      },
...
async function checkChatWebhook(url: string, timeout: number = 15, message: string = 'Hola'): Promise<any> {
  ...
  try {
    ...
    const body = JSON.stringify({
      sessionId: '1118e592-0b84-459e-bf19-39131f578869',
      chatInput: message
    });
    ...
    try {
      ...
      if (reply === null) {
        reply = responseText.length > 200 ? responseText.substring(0, 200) + '...' : responseText;
      }
    } catch {
      reply = responseText.length > 200 ? responseText.substring(0, 200) + '...' : responseText;
    }
...
async function checkN8nWorkflowActive(baseUrl: string, workflowId: string, apiKey: string, timeout: number = 10): Promise<{ active: boolean; error: string | null; status_code: number | null }> {
  try {
    ...
    if (response.status !== 200) {
      return { active: false, error: `status_code=${response.status}`, status_code: response.status };
    }

    const data = await response.json();
    const wf = data.data || data;
    const activeVal = wf.active ?? wf.isActive ?? wf.is_active ?? null;
    const active = activeVal === true || activeVal === 1 || activeVal === '1' || activeVal === true;

    return { active, error: null, status_code: response.status };
  } catch (error: any) {
    return { active: false, error: error.message, status_code: null };
  }
}
...
async function checkCartaLaboral(cartaUrl: string, timeout: number = 30, nit: string = '1095796088'): Promise<any> {
  ...
  checks.push({
    label: '1) Workflow (API)',
    ok: wfCheck.active,
    url: wfUrl,
    error: wfCheck.active ? null : (wfCheck.error || 'inactivo'),
    status_code: wfCheck.status_code
  });
  ...
  try {
    ...
    checks.push({
      ...
      ok: sighaOk,
      url: sighaLoginUrl,
      error: sighaOk ? null : `status_code=${sighaCode}`,
      status_code: sighaCode
    });
  } catch (e: any) {
    checks.push({
      label: '2) Sigha',
  ...
  try {
    const cartaResponse = await fetch(cartaUrl, {
      ...
      headers: {
        ...
        'x-api-key': await getModuleConfig('carta_api_key') || ''
      },
    ...
    checks.push({
      label: '3) carta-laboral',
      ok: cartaOk,
      url: cartaUrl,
      error: cartaOk ? null : (parsed?.message || `status_code=${cartaCode}`),
      status_code: cartaCode
    });
  } catch (e: any) {
    checks.push({
      label: '3) carta-laboral',
...
async function checkConocimientoMarcas(urls: string[], timeout: number = 15): Promise<any> {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      const response = await fetch(url, {
        method: 'HEAD',
        signal: AbortSignal.timeout(timeoutPerUrl * 1000)
      });
      ...
      checks.push({
        ...
        ok,
        url,
        error: ok ? null : `status_code=${response.status}`,
  ...
  return {
    status: allOk ? 'online' : 'offline',
    http_code: checks[checks.length - 1]?.status_code || 0,
    response_time_ms: Date.now() - startTime,
    conocimiento_marcas_checks: checks,
    error: null
  };
}
...
async function checkWebSeguridad(url: string, timeout: number = 10): Promise<any> {
  ...
  try {
    ...
    const webOk = response.status === 200;
    checks.push({
      label: 'Web',
      ok: webOk,
      url,
      error: webOk ? null : `status_code=${response.status}`,
      status_code: response.status
    });
  } catch (e: any) {
    checks.push({
  ...
  checks.push({
    label: 'Redireccion (5 llamadas)',
    ok: badCount === 0,
    url,
    error: badCount === 0 ? null : `${badCount} de 5 sin 200: ${Array.from(new Set(badCodes)).slice(0, 3).join(',')}`,
    status_code: badCount === 0 ? 200 : (badCodes[0] || null)
  });

  const allOk = checks.every(c => c.ok);
  return {
    status: allOk ? 'online' : 'offline',
    http_code: checks[0]?.status_code || 0,
    response_time_ms: Date.now() - startTime,
    web_seguridad_checks: checks,
    error: null
  };
}

async function checkOutlookConnection(timeout: number = 15): Promise<any> {
  const startTime = Date.now();
  const clientId = await getModuleConfig('outlook_client_id');
  const clientSecret = await getModuleConfig('outlook_client_secret');
  ...
  if (!clientId || !clientSecret) {
    return {
      ...
      http_code: 0,
      response_time_ms: Date.now() - startTime,
      error: 'Sin credenciales de Outlook configuradas'
    };
  }
  ...
  try {
    ...
    return {
      status: response.status === 200 && hasToken ? 'online' : 'offline',
      http_code: response.status,
      response_time_ms: responseTime,
      error: response.status !== 200 ? (data.error_description || `status_code=${response.status}`) : null
    };
  } catch (error: any) {
    return {
      status: 'offline',
...
async function getN8nLastExecution(baseUrl: string, workflowId: string, apiKey: string, timeout: number = 10): Promise<any> {
  try {
    const url = `${baseUrl}/api/v1/executions?workflowId=${workflowId}&limit=1`;
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'X-N8N-API-KEY': apiKey,
    ...
    if (executions.length > 0) {
      ...
      return {
        status: last.status || last.executionStatus || last.finished || null,
        finished_at: last.stoppedAt || last.finishedAt || last.startedAt || null,
        error: false
      };
    }
...
async function checkConocimientoIntranet(baseUrl: string, timeout: number = 15, search: string = 'test'): Promise<any> {
  ...
  try {
    ...
    checks.push({
      label: '2) API Intranet',
      ok: apiOk,
      url,
      error: apiOk ? null : `status_code=${response.status}`,
      status_code: response.status
    });
  } catch (e: any) {
    checks.push({
...
async function checkDatosUsuarios(url: string, timeout: number = 15): Promise<any> {
  ...
  try {
    ...
    return {
      ...
      http_code: response.status,
      response_time_ms: Date.now() - startTime,
      datos_usuarios_search_term: 'Juan ballesteros',
      error: null
    };
  } catch (error: any) {
    return {
...
async function checkVcardsSearch(url: string, timeout: number = 15): Promise<any> {
  ...
  try {
    const body = JSON.stringify({ search: searchName });
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Authorization': `Basic ${tarjetavAuth}`,
...
async function checkFormularioVcard(baseUrl: string, timeout: number = 15): Promise<any> {
  ...
  if (!vcardId) {
    return {
      ...
      http_code: 0,
      response_time_ms: Date.now() - startTime,
      error: 'No se obtuvo id de vcards'
    };
  }

  try {
    const url = `${baseUrl}${vcardId}`;
    const response = await fetch(url, {
      method: 'HEAD',
      signal: AbortSignal.timeout(timeout * 1000)
    });
...
async function verifyService(service: any, sectionIndex: number, serviceIndex: number): Promise<any> {
  const baseResult = {
    ...
    en_desarrollo: service.en_desarrollo || false
  };

  if (service.en_desarrollo) {
    return { ...baseResult, status: 'en_desarrollo', response_time_ms: null, error: null };
  }
  ...
  switch (service.type) {
    case 'http': {
      ...
      if (service.test_carta_laboral) {
        const result = await checkCartaLaboral(service.endpoint, service.timeout, service.carta_nit);
        return { ...baseResult, ...result, test_carta_laboral: true };
      }
      if (service.test_conocimiento_general) {
        const result = await checkConocimientoGeneral(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_conocimiento_general: true };
      }
      if (service.test_conocimiento_marcas) {
        const result = await checkConocimientoMarcas(service.conocimiento_marcas_urls || [], service.timeout);
        return { ...baseResult, ...result, test_conocimiento_marcas: true };
      }
      if (service.test_web_seguridad) {
        const result = await checkWebSeguridad(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_web_seguridad: true };
      }
      if (service.test_conocimiento_intranet) {
        const result = await checkConocimientoIntranet(service.endpoint, service.timeout, 'test');
        return { ...baseResult, ...result, test_conocimiento_intranet: true };
      }
      if (service.test_birthday_search) {
        const result = await checkBirthdaySearch(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_birthday_search: true };
      }
      if (service.test_datos_usuarios) {
        const result = await checkDatosUsuarios(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_datos_usuarios: true };
      }
      if (service.test_vcards_search) {
        const result = await checkVcardsSearch(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_vcards_search: true };
      }
      if (service.test_formulario_vcard) {
        const result = await checkFormularioVcard(service.endpoint, service.timeout);
        return { ...baseResult, ...result, test_formulario_vcard: true };
      }

      const headers: Record<string, string> = {};
      if (service.headers) {
        for (const [key, value] of Object.entries(service.headers as Record<string, string>)) {
          if (typeof value === 'string' && value.includes('require')) {
            const n8nKey = await getModuleConfig('n8n_api_key');
            if (n8nKey) {
              headers[key.replace(/require\s*\(\s*['"`][^"'`]+['"`]\s*\)/, '').trim()] = n8nKey;
            }
          } else {
            headers[key] = value;
          }
      ...
      const result: any = await checkHttpService(service.endpoint, service.timeout, {
        ...
        accept_codes: service.accept_codes,
        accept_400: service.accept_400,
        headers
      });
      ...
      if (service.fetch_executions) {
        const n8nApiKey = await getModuleConfig('n8n_api_key');
        if (n8nApiKey) {
          const exec = await getN8nLastExecution(
            service.executions_base_url || 'https://automation.zeroazul.com',
            service.workflow_id || '',
            n8nApiKey,
            service.timeout
          );
    ...
    case 'outlook': {
      const result = await checkOutlookConnection(service.timeout);
      return { ...baseResult, ...result };
    }
...
export async function GET() {
  try {
    ...
    for (let sectionIndex = 0; sectionIndex < SERVICE_SECTIONS.length; sectionIndex++) {
      const section = SERVICE_SECTIONS[sectionIndex];
      const sectionServices: any[] = [];
    ...
    console.error('[AUTOLARTE-SERVICES]', error);
