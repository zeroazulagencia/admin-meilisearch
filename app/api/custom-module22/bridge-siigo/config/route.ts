...
import { getAllConfig, setConfig } from '@/utils/modulos/bridge-siigo/module22-config';
...
export async function GET() {
  try {
    ...
    console.error('[BridgeSiigo-Config-GET] Error:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('[BridgeSiigo-Config-PUT] Error:', error);
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }
}