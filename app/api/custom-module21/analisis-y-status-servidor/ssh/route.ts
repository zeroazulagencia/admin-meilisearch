...
'wp-file-audit': `echo "=== DETAILS ==="; find /var/www/comunicacionesnew/ -type f -size +5M ! -path '*/cache/*' ! -path '*/tmp/*' ! -path '*/.git/*' 2>/dev/null | while IFS= read -r f; do size=$(stat --format="%s" "$f" 2>/dev/null); mtime=$(stat --format="%y" "$f" 2>/dev/null); ext=$(echo "$f" | awk -F. '{print $NF}'); echo "$ext|$size|$mtime|$f"; done | sort -t"|" -k2 -rn | head -100; echo "---"; echo "=== BY_TYPE ==="; find /var/www/comunicacionesnew/ -type f -size +5M ! -path '*/cache/*' ! -path '*/tmp/*' ! -path '*/.git/*' 2>/dev/null | while IFS= read -r f; do size=$(stat --format="%s" "$f" 2>/dev/null); ext=$(echo "$f" | awk -F. '{print $NF}'); echo "$ext|$size"; done | awk -F'|' '{ext=$1; sz=$2; if(ext ~ /^(jpg|jpeg|png|gif|webp|bmp|svg|ico)$/) c="images"; else if(ext ~ /^(pdf)$/) c="pdf"; else if(ext ~ /^(zip|tar|gz|bz2|xz|7z|rar|tgz)$/) c="archive"; else if(ext ~ /^(mp4|avi|mkv|mov|wmv|flv)$/) c="video"; else if(ext ~ /^(mp3|wav|ogg|flac|aac|wma)$/) c="audio"; else c="other"; a[c]+=sz; n[c]++} END{for(c in a) print c "|" a[c] "|" n[c]}'; echo "---"; echo "=== WP_SIZE ==="; du -sb /var/www/comunicacionesnew/ 2>/dev/null | awk '{print $1}'; echo "---"; echo "=== DISK ==="; df -B1 / 2>/dev/null | tail -1 | awk '{print $2 "|" $3 "|" $5}'; echo "---"; echo "=== SUMMARY ==="; find /var/www/comunicacionesnew/ -type f -size +5M ! -path '*/cache/*' ! -path '*/tmp/*' ! -path '*/.git/*' 2>/dev/null | wc -l`,
  'crit-precheck': `php_svc=$(systemctl list-units --type=service 2>/dev/null | grep -oP 'php[\\d.]+-fpm\\.service' | head -1 | sed 's/\\.service//'); echo "=== NGINX_TEST ==="; nginx -t 2>&1; echo "---"; echo "=== SERVICES ==="; for s in nginx mysql mariadb; do echo "$s: $(systemctl is-active $s 2>/dev/null || echo not_found)"; done; if [ -n "$php_svc" ]; then echo "php-fpm: $(systemctl is-active "$php_svc" 2>/dev/null || echo not_found)"; else echo "php-fpm: not_found"; fi; echo "---"; echo "=== LOAD ==="; uptime; echo "---"; echo "=== DISK ==="; df -h / | tail -1; echo "---"; echo "=== MEMORY ==="; free -h | grep Mem`,
'crit-kill': `php_svc=$(systemctl list-units --type=service 2>/dev/null | grep -oP 'php[\\d.]+-fpm\\.service' | head -1 | sed 's/\\.service//'); echo "=== BEFORE ==="; echo "PHP-FPM workers:"; pgrep -ac php-fpm 2>/dev/null || echo 0; echo "---"; echo "=== ZOMBIES ==="; ps aux | grep -w Z | grep -v grep; echo "---"; echo "=== RESTARTING PHP-FPM ==="; if [ -n "$php_svc" ]; then systemctl restart "$php_svc" 2>&1; else systemctl restart php*-fpm 2>&1; fi; echo "---"; echo "=== AFTER ==="; if [ -n "$php_svc" ]; then systemctl status "$php_svc" --no-pager 2>/dev/null | head -8; else systemctl status php*-fpm --no-pager 2>/dev/null | head -8; fi; echo "---"; pgrep -ac php-fpm 2>/dev/null || echo 0`,
  'crit-restart-nginx': `echo "=== FIX CERTS ==="; chmod 755 /etc/letsencrypt/live/ 2>/dev/null; chmod 755 /etc/letsencrypt/archive/ 2>/dev/null; find /etc/letsencrypt -type d -exec chmod 755 {} \; 2>/dev/null; find /etc/letsencrypt -type f -exec chmod 644 {} \; 2>/dev/null; echo "Permisos corregidos"; echo "---"; echo "=== NGINX_TEST ==="; nginx -t 2>&1; echo "---"; if nginx -t 2>&1 | grep -qi "syntax is ok"; then echo "REINICIANDO NGINX..."; systemctl restart nginx 2>&1 && (echo "---"; echo "OK: Nginx reiniciado"); echo "---"; echo "=== STATUS ==="; systemctl status nginx --no-pager 2>/dev/null | head -8; else echo "ERROR: Config de nginx con errores. No se reinicia."; fi`,
};// ── Rate limiter ──
...
function parseCritPrecheck(raw: string): StructuredBlock[] {
  ...
  const phpActive = /php[\d.]*-fpm.*active/.test(raw) || /php-fpm:\s+active/.test(raw);
  blocks.push({
